...
def render_team_map(header, people, today):
    ...
    out = [
        ...
        "---", "", "# MediaHub Team Map", "",
        "What this is: the roster — the only place a person's identity (email, Slack id, Jira account) is asserted. "
...
def cmd_roster(a):
    ...
    try:
        ...
        tmp = os.path.join(dest_dir, "team-map.md.tmp")
        open(tmp, "w", encoding="utf-8").write(render_team_map(header, people, today))
        os.replace(tmp, os.path.join(dest_dir, "team-map.md"))
    except OSError as e:
        print(f"cannot write {dest_dir}: {e}", file=sys.stderr); sys.exit(EX_WRITE)
    mpath, row = update_manifest(records, src_rel, "knowledge/products/mediahub/team/ (roster.yaml, team-map.md)", today,
                                 header.get("verified", "?"), "Helm `known_emails()` and its roster parser; `ela who` on the remote site")
    if a.json:
    ...
    print(f"published knowledge/products/mediahub/team/ (roster.yaml, team-map.md)\n  {len(people)} people · source verified {header.get('verified')} · published {today}\n  manifest row → {mpath}\n  {row}")
