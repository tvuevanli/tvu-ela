...
# ── the roster: knowledge/products/mediahub/team/roster.yaml → team-map.md + the yaml as it is ─────

def render_team_map(header, people, today):
    """A document a reader (or Helm's roster parser) can use: one block per person, identity lines in the shape
    Helm already parses — `**Name** — Role`, `- Email:`, `- Slack:`, `- Jira accountId:`, `- Jira: `displayName` — accountId `…``."""
    areas = []
    for p in people:
    ...
    out = [
        ...
        "source: generated by `ela publish roster` from elak knowledge/products/mediahub/team/roster.yaml — edit the yaml, publish again",
        "---", "", "# MediaHub Roster", "",
        "What this is: the roster — who is on the team and how each person is identified. It is not Helm's team-map.md "
        "(ownership rules, status roles, external owners), which stays Helm's until it is admitted to elak. The only place a person's identity (email, Slack id, Jira account) is asserted. "
        "Every email and Slack id was read first-hand from Slack `users.list`; Jira ids were confirmed against ticket history. "
    ...
    for area in areas:
        out += [f"## {area}", ""]
        for p in [x for x in people if x["area"] == area]:
            out.append(f"**{p['name']}** — {p['role']}" + (" (external, cross-team)" if p.get("external") else ""))
            if p.get("layer"):
                out.append(f"- Layer: {p['layer']}")
            if p.get("reports_to"):
                out.append(f"- Reports to: {p['reports_to']}")
...
def cmd_roster(a):
    ...
    records, published = roots()
    src_rel = "knowledge/products/mediahub/team/roster.yaml"
    src = os.path.join(records, src_rel)
    header, people = team.read_roster(src)
    if not people:
        print(f"{src}: no people parsed", file=sys.stderr); sys.exit(EX_USAGE)
    today = datetime.date.today().isoformat()
    dest_dir = os.path.join(published, "knowledge", "products", "mediahub", "team")
    try:
        os.makedirs(dest_dir, exist_ok=True)
        shutil.copyfile(src, os.path.join(dest_dir, "roster.yaml"))
        ...
        stale = os.path.join(dest_dir, "team-map.md")      # published under Helm's filename until 2026-09-03; a different document
        if os.path.exists(stale):
            os.remove(stale)
    except OSError as e:
        print(f"cannot write {dest_dir}: {e}", file=sys.stderr); sys.exit(EX_WRITE)
    mpath, row = update_manifest(records, src_rel, "knowledge/products/mediahub/team/ (roster.yaml, roster.md)", today,
    ...
    print(f"published knowledge/products/mediahub/team/ (roster.yaml, roster.md)\n  {len(people)} people · source verified {header.get('verified')} · published {today}\n  manifest row → {mpath}\n  {row}")
