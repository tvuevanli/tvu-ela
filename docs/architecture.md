...
| **published directory** | the output of `ela publish`: the map subset, the service catalogue, the roster | shareable inside the company | anything not produced by `publish.py`; it is deleted and regenerated, never edited |
