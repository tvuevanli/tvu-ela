...
| **L1** | *use it*: `/ela:brief` every morning; `/ela:breakdown` on one real requirement; Helm's `clients/ela.py` seam (branch `ela-integration`) with the UR reads as first entries | two weeks of misses written (`elak/records/misses/`, one file per day that has one); one plan diffed against Evan's; the seam's tests green | seam done; **the misses record opened 2026-09-07** with the day's entry, and the Slack lanes the brief stands on were broken until the same day (ela 0.30.0) — the two weeks start from a working brief |
