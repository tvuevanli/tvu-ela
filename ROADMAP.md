...
| **L4** | sink Helm's judgments into ela one at a time (Daily Report, My Report, triage, evidence rules) — needs a `mail` sense first | one Helm prompt removed per week; readers notice nothing | not started |
| **L5** | composites over the atoms: nudge, scope tracker, ticket-from-thread, decision capture, promotion post, long-thread digest | the third follow-up on a bug is never hand-written | not started |
